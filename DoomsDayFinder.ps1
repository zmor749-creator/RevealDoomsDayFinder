...
$script:SignatureUrl = 'https://raw.githubusercontent.com/zmor749-creator/DoomsDayFinder/main/signatures/doomsday.json'
...
function Write-Stage {
    param([string]$Stage, [string]$Message, [ConsoleColor]$Color = [ConsoleColor]::DarkCyan)
    Write-Color (('[{0}] {1}' -f $Stage.ToUpperInvariant(), $Message)) $Color
}

function Show-LionBanner {
    $lion = @'
                    .-""""""""-.
                  .'   _      _   '.
                 /    / \    / \    \
                ...
                |     \  ____  /     |
                 \      '.__.'      /
                  ...
                  [ LION FORENSICS ]
'@
    Write-Host $lion -ForegroundColor Yellow
    Write-Host '                 DOOMSDAY FINDER' -ForegroundColor Cyan
    Write-Host '              FORENSIC DETECTION ENGINE' -ForegroundColor DarkCyan
...
function Invoke-FileInspection {
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$LiteralPath, $State, [string]$Source = 'FILE', [switch]$AlwaysRecord)
    if (-not (Test-Path -LiteralPath $LiteralPath -PathType Leaf)) { throw "File not found: $LiteralPath" }
    $State.CandidateFiles++
    $State.FilesFound++
    $item = Get-Item -LiteralPath $LiteralPath -Force
    Write-Stage 'HASH' "Computing SHA-256: $($item.Name)"
    try {
        ...
        $jar = $null
        if ($magic.ActualType -eq 'Java Archive / ZIP') {
            Write-Stage 'JAR' "Inspecting every archive entry: $($item.Name)"
            ...
            Write-Stage 'CLASS' ("{0} / {0} classes" -f $jar.ClassCount)
...
function Invoke-CandidateScan {
    ...
    foreach ($file in $files) {
        $index++
        Write-Stage 'FILE' ("{0} / {1}: {2}" -f $index, $files.Count, $file)
        Invoke-FileInspection -LiteralPath $file -State $State | Out-Null
    }
}
    ...
    try {
        ...
        foreach ($process in $processes) {
            ...
            $record = [pscustomobject][ordered]@{
                ...
                StartTime = try { [Management.ManagementDateTimeConverter]::ToDateTime($process.CreationDate).ToUniversalTime() } catch { $null }
            ...
            try {
                ...
                try {
                    foreach ($module in $native.Modules) {
                        ...
                        $hash = try { Get-CachedFileSha256 $modulePath } catch { '' }
                        [void]$record.LoadedModules.Add([pscustomobject][ordered]@{
                            ...
                            Size = try { (Get-Item -LiteralPath $modulePath).Length } catch { 0 }; PID = $record.PID
                        })
                    }
    ...
    try {
        ...
        if ($service.ProcessId -gt 0) {
            try {
                ...
                $record.SvchostStartTime = try { [Management.ManagementDateTimeConverter]::ToDateTime($hostProcess.CreationDate).ToUniversalTime() } catch { $null }
...
function Invoke-AdsScan {
    ...
    $seen = @{}
    foreach ($file in $files) {
        if ($seen.ContainsKey($file)) { continue }; $seen[$file]=$true
        try {
            ...
            foreach ($streamInfo in $streams) {
                ...
                Write-Stage 'ADS' "Inspecting named stream $file`:$name"
...
function Get-DefaultAdsRoots {
    ...
    if ($env:LOCALAPPDATA) { [void]$roots.Add($env:LOCALAPPDATA) }
    if ($env:APPDATA) { [void]$roots.Add($env:APPDATA) }
...
function Show-FindingsTable {
    param($State)
    if ($State.Findings.Count -eq 0) { return }
    Write-Section 'FINDINGS TABLE'
    Write-Color ('{0,-14} | {1,4} | {2,-11} | {3,-8} | {4,-15} | {5,-5} | {6,-30} | {7}' -f 'STATUS','CONF','VERIFY','FAMILY','TYPE','EXT','FILE','SOURCE') Cyan
    foreach ($finding in @($State.Findings | Sort-Object @{Expression='Confidence';Descending=$true})) {
...
function Show-ScanComplete {
    ...
    foreach ($key in $rows.Keys) { Write-Color (('{0,-26}: {1}' -f $key, $rows[$key])) Gray }
    Write-Host ''
    ...
    foreach ($finding in @($State.Findings | Where-Object { $_.Verdict -in @('DETECTED','HIGH CONFIDENCE','SUSPICIOUS','INCONCLUSIVE','DELETED TRACE','RUNTIME TRACE') })) { Show-FindingDetail $finding }
}

function Convert-ReportToText {
    param($State)
    $builder = [Text.StringBuilder]::new()
    [void]$builder.AppendLine('=' * 60); [void]$builder.AppendLine('DOOMSDAY FINDER REPORT'); [void]$builder.AppendLine('=' * 60)
    ...
    [void]$builder.AppendLine("Started UTC: $($State.StartedUtc.ToString('o'))")
    [void]$builder.AppendLine("Completed UTC: $($State.CompletedUtc.ToString('o'))")
...
function Export-ScanReport {
    ...
    $stamp = $State.CompletedUtc.ToString('yyyyMMdd_HHmmss')
    ...
    Write-Color "[REPORT] $jsonPath" Cyan; Write-Color "[REPORT] $textPath" Cyan
...
function Invoke-ScanMode {
    ...
    switch ($SelectedMode) {
        ...
        'ADS' {
            Write-Section 'DEEP ADS SCAN'; $roots = if ($SelectedPath) { @($SelectedPath) } else { Get-DefaultAdsRoots }
...
function Show-MainMenu {
    ...
    Show-LionBanner
    Write-Color '[1] Quick Scan' Gray
    Write-Color '[2] Full Forensic Scan' Gray
    Write-Color '[3] Scan File' Gray
    Write-Color '[4] Deep ADS Scan' Gray
    Write-Color '[5] Runtime Scan' Gray
    Write-Color '[6] Update Signatures' Gray
    Write-Color '[7] Export Last Report' Gray
